...
This document provides practical examples for using the Figma Comments API client in various scenarios, including the new reaction management features and advanced configuration options.
...
// Simple initialization with API token
...
const sdk = new FigmaCommentsSDK({
  ...
  // API endpoint (optional, defaults to https://api.figma.com)
  baseUrl: 'https://api.figma.com',

  // Logging (optional)
  logger: console,  // or custom logger: { debug, info, warn, error }

  // Request timeout (optional, default: 30000ms)
  timeout: 60000,
  ...
  // Caching (optional, enabled by default for GET requests)
  cache: {
...
process.env.HTTP_PROXY_TOKEN = 'proxy-auth-token';  // If proxy requires authentication
...
const sdk = new FigmaCommentsSDK({
  ...
  proxyToken: 'proxy-auth-token'  // Optional
});
```
...
const sdk = new FigmaCommentsSDK({
  apiToken: process.env.FIGMA_TOKEN,
  fetchFunction: customFetchAdapter  // Your custom fetch implementation
});
```

### Disable Rate Limiting and Caching

```javascript
// For scenarios where you want full control
const sdk = new FigmaCommentsSDK({
  apiToken: process.env.FIGMA_TOKEN,
  rateLimitConfig: null,  // Disable rate limiting
...
```javascript
// Example for corporate networks with proxy and custom certificates
import FigmaCommentsSDK from 'figma-comments';
